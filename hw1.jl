...
random_vect = rand(1:9, 10) # replace this with your code!
...
function vecvec_to_matrix(vecvec)
	...
	res = reshape(res, (length(vecvec[1]), length(vecvec)))
	return res
end
...
function mean_colors(image)
	(h, w) = size(image)
	red_mean = sum(+, [image[i, j].r for i=1:h, j=1:w]) / (w*h)
	grn_mean = sum(+, [image[i, j].g for i=1:h, j=1:w]) / (w*h)
	blu_mean = sum(+, [image[i, j].b for i=1:h, j=1:w]) / (w*h)
	return (red_mean, grn_mean, blu_mean)
end
...
begin
	...
	function quantize(image::AbstractMatrix)
		...
		return (quantize(image[i, j]) for i=1:w, j=1:h)
	end
end
...
function invert(color::AbstractRGB)
	return (1 - color.r, 1 - color.g, 1 - color.b)
end
...
begin
	...
	function noisify(color::AbstractRGB, s)
		# you will write me in a later exercise!
		return (noisify(color.r, s), noisify(color.g, s), noisify(color.b, s))
	end
	...
	function noisify(image::AbstractMatrix, s)
		...
		return (noisify(image[i, j]) for i=1:w, j=1:h)
	end
end
...
function blur_1D(v, l)
	t = copy(v)
	for i in length(t)
		t[i] = mean([extend(v, j) for j=i-l:i+l])
	end
	return t
end
...
# ╔═╡ f3b2f4d8-3bdb-11eb-178b-e9463cb35f99
@bind l_box Slider(0:1:80, show_value=true)
...
function convolve_vector(v, k)
	...
	l = Int((length(k) - 1) / 2)
	for i in 1:length(v)
		append!(t, sum([extend(v, i-j) * k[j+l+1] for j=-l:l]))
	end
	return t
end
...
function gaussian_kernel(n)
	...
	return exp(-(n^2)/2)/(sqrt(2*pi))
end

# ╔═╡ f8bd22b8-ee14-11ea-04aa-ab16fd01826e
md"Let's test your kernel function!"

# ╔═╡ 5ac429e0-3c92-11eb-0686-a96c3dc0849f
@bind gaussian_kernel_slider Slider(0:1:8, show_value=true)

# ╔═╡ 2a9dd06a-ee13-11ea-3f84-67bb309c77a8
gaussian_kernel_size_1D = gaussian_kernel_slider # change this value, or turn me into a slider!
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	for x in 1:m_rows, y in 1:m_columns
		sum = RGB()
		for i in 1:num_rows, j in 1:num_columns
			sum += extend_mat(M, x-i, y-j) * K[i, j]
		t[x, y] = sum
		end
	end
	return t
...
# ╔═╡ ea00d4fc-3ca8-11eb-0959-87af093e4102
function gaussian_2d_kernel(x, y)
	return exp(-1*((x^2+y^2)/2)/(2*pi))
end

# ╔═╡ caf29494-3ca9-11eb-3869-956e2f4e680d
@bind gaussian_kernel_slider_2d Slider(0:1:8, show_value=true)

# ╔═╡ aad67fd0-ee15-11ea-00d4-274ec3cda3a3
function with_gaussian_blur(image)
	#(x, y) = size(image)
	k = [gaussian_2d_kernel(x, y) for x=-gaussian_kernel_slider_2d:gaussian_kernel_slider_2d, y=-gaussian_kernel_slider_2d:gaussian_kernel_slider_2d]
	k = k ./ sum(k)
	
	if k !== missing
		ci = convolve_image(image, k)
	end
	return ci
end
