...
# ╔═╡ 467ada64-485c-11eb-294e-31e05d56aae4
ocean_24h_sim = let
	P = OceanModelParameters(κ=κ_ex)
...
# ╔═╡ 7bdc4024-48d8-11eb-1e41-ff545b81ed3a
md"""
$\frac{S(x,y)(1 - \alpha(T))}{4C}$ 
"""
...
function absorbed_solar_radiation(T::Array{Float64,2}, model::RadiationOceanModel)
	S = S_at.(model.grid.y[:]; model.grid, model.params.S_mean) .* (1 .- α(T, model)) ./ (4 * model.params.C)
	return S
end
...
# ╔═╡ 9efe70c2-48db-11eb-2663-a1dcbc5cbae0
md"""
$-\frac{(A - BT)}{C}$
"""

# ╔═╡ eda5fe76-4862-11eb-144e-b3c930cd2dec
function outgoing_thermal_radiation(T::Array{Float64,2}, model::RadiationOceanModel)
	
	return outgoing_thermal_radiation(T; C=model.params.C, A=model.params.A, B=model.params.B)
end
...
function runtime(N)
	...
	Δt = 12*60*60
	ocean_sim = ClimateModelSimulation(model, copy(ocean_T_init), Δt)
	runtime = 0
	runtime = @elapsed timestep!(ocean_sim)
	return runtime
end
...
# ╔═╡ 4781e0b2-48dc-11eb-0b3a-75f231080f91
absorbed_solar_radiation(radiation_sim.T, radiation_sim.model)

# ╔═╡ 3e11cede-48dc-11eb-2cd7-9b5c3f2d8ab3
outgoing_thermal_radiation(radiation_sim.T, radiation_sim.model)
...
# ╔═╡ 651e7a02-48de-11eb-2443-3f8be9932e3d
md"""
$\begin{split}
C\frac{\partial T}{\partial t} =&\ \hphantom{+} 
\frac{S(1 - \alpha)}{4} 
...
function eq_T(S, T_init_value)
	...
	while status
		...
		ioR = S_at.(model.grid.y[:]; model.grid, model.params.S_mean) / 4 .- (model.params.A .- model.params.B * sim.T)
		if abs(sum(ioR)) ≤ 0.1
			status = false
		end
	end
...
# ╔═╡ bf56acd2-485d-11eb-3606-9f577ea96101
let
	go_ex
	if ocean_24h_sim.iteration == 0
		timestep!(ocean_24h_sim)
	else
		for i in 1:50
			timestep!(ocean_24h_sim)
		end
	end
	plot_state(ocean_24h_sim, clims=(-10, 40), show_quiver=show_quiver, show_anomaly=show_anomaly, IC=ocean_T_init)
end
