...
About 30% allocs reduced.
vcat() does copy but we don't
"""
...
As @view, we just have allo reference
"""
...
# ╔═╡ 10ee8610-3d2e-11eb-0ab0-41d283697267
function greedy_seam_edge_fix(energies, col_index)
	m, n = size(energies)
	if col_index < 1
		return 1
	elseif col_index > n
...
function greedy_seam(energies, starting_pixel::Int)
	...
	# return random_seam(size(energies)..., starting_pixel)
	(num_row, num_column) = size(energies)
	greedy_seam = zeros(Int, num_row)
	greedy_seam[1] = starting_pixel
	for i in 2:num_row
		left = greedy_seam_edge_fix(energies, greedy_seam[i-1] - 1)
		right = greedy_seam_edge_fix(energies, greedy_seam[i-1] + 1)
		idx = findmin([energies[i, j] for j in left:right])[2]
		len = right - left + 1
		if len == 3
			greedy_seam[i] = greedy_seam_edge_fix(energies, greedy_seam[i-1] + idx - 2)
		elseif len == 2 && right == num_column
			greedy_seam[i] = greedy_seam_edge_fix(energies, greedy_seam[i-1] + idx - 2)
		elseif len == 2 && left == 1
			greedy_seam[i] = greedy_seam_edge_fix(energies, greedy_seam[i-1] + idx - 1)
		end
	end
	return greedy_seam
end
...
function least_energy(energies, i, j)
    ...
    sw_e = least_energy(energies, i + 1, clamp(j - 1, 1, col_column))[1]
    s_e = least_energy(energies, i + 1, j)[1]
    se_e = least_energy(energies, i + 1, clamp(j + 1, 1, col_column))[1]
    min_e, idx = findmin([sw_e, s_e, se_e])
    t_e = energies[i,j] + min_e
    return (t_e, clamp(j + idx - 2, 1, col_column))
end
...
function recursive_seam(energies, starting_pixel)
	...
	recur_seam = zeros(Int, m)
	recur_seam[1] = starting_pixel
	for i in 2:m
		recur_seam[i] = least_energy(energies, i, recur_seam[i-1])[2]
	end
	return recur_seam
end
...
function matrix_memoized_least_energy(energies, i, j, memory)
    ...
    if memory[i, j] !== 0
        ...
        sw_e = matrix_memoized_least_energy(energies, i + 1, clamp(j - 1, 1, n), memory)[1]
        s_e = matrix_memoized_least_energy(energies, i + 1, j, memory)[1]
        se_e = matrix_memoized_least_energy(energies, i + 1, clamp(j + 1, 1, n), memory)[1]
        min_e, idx = findmin([sw_e, s_e, se_e])
        t_e = energies[i,j] + min_e
        memory[i, j] = t_e
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	seam[1] = starting_pixel
	for i in 2:m
		seam[i] = matrix_memoized_least_energy(energies, i, seam[i-1], memory)[2]
	end
	return seam
end
