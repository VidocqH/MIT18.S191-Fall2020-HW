...
# ╔═╡ 5b46caec-4209-11eb-2a07-39029be65945
function unused_letters_func()
	res = []
	for i in 'a':'z'
		if sample_freqs[index_of_letter(i)] == 0.0
			append!(res, i)
		end
	end
	if sample_freqs[27] == 0.0
		append!(res, ' ')
	end
	return res
end
...
unused_letters = unused_letters_func()
...
# ╔═╡ ca2c28e8-420a-11eb-2195-bfe72d5fc5b3
function double_lett(freq_matrix)
	res = []
	for i in 'a':'z'
		if freq_matrix[index_of_letter(i), index_of_letter(i)] ≠ 0.0
			append!(res, i)
		end
	end
	if freq_matrix[index_of_letter(' '), index_of_letter(' ')] ≠ 0.0
		append!(res, ' ')
	end
	return res
end
...
double_letters = double_lett(sample_freq_matrix)
...
# ╔═╡ 0d5fa036-420b-11eb-0adc-79054cc97956
function most_follow_w(freq_matrix)
	maxVal = 0.0
	res = 'a'
	for i in 'a':'z'
		if freq_matrix[index_of_letter('w'), index_of_letter(i)] > maxVal
			maxVal = freq_matrix[index_of_letter('w'), index_of_letter(i)]
			res = i
		end
	end
	if freq_matrix[index_of_letter('w'), index_of_letter(' ')] > maxVal
		res = ' '
	end
	return res
end
...
most_likely_to_follow_w = most_follow_w(sample_freq_matrix)
...
# ╔═╡ b43dbc26-420b-11eb-23c6-697b158ca092
function most_precede_w(freq_matrix)
	maxVal = 0.0
	res = 'a'
	for i in 'a':'z'
		if freq_matrix[index_of_letter(i), index_of_letter('w')] > maxVal
			maxVal = freq_matrix[index_of_letter(i), index_of_letter('w')]
			res = i
		end
	end
	if freq_matrix[index_of_letter(' '), index_of_letter('w')] > maxVal
		res = ' '
	end
	return res
end
...
most_likely_to_precede_w = most_precede_w(sample_freq_matrix)
...
function matrix_distance(A, B)
	subtract_matrix = A .- B # do something with A .- B
	subtract_matrix = abs.(subtract_matrix)
	return sum(subtract_matrix)
end
...
# ╔═╡ df1d94f4-420e-11eb-2070-33a537a46afa
eltype([1, 2, 3, 42])

# ╔═╡ 7be98e04-fb6b-11ea-111d-51c48f39a4e9
function ngrams(words, n)
	res = Array{Array{eltype(words), 1}, 1}()
	for i in 1:length(words)-n+1
		append!(res, [words[i:i+n-1]])
	end
	return res
end
...
function completions_cache(grams)
	...
	for i in 1:length(grams)
		key = [grams[i][j] for j in 1:n-1]
		if haskey(cache, key)
			append!(cache[key], [grams[i][n]])
		else
			cache[key] = []
			append!(cache[key], [grams[i][n]])
		end
