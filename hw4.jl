...
student = (name = "Vidocq", kerberos_id = "vidocqh")
...
# ╔═╡ d2c8d68a-4224-11eb-0ebc-e3a7cd317cc4
@bind p_interactive Slider(0.01:0.01:1, default=0.25, show_value=true)

# ╔═╡ bb63f3cc-042f-11eb-04ff-a128aec3c378
@bind N_interactive Slider(1:1:100_000, default=10_000, show_value=true)

# ╔═╡ 395a1556-4227-11eb-0237-61ba017b23ed
...
@bind p Slider(0.001:0.001:1, default=0.250, show_value=true)

# ╔═╡ a4881142-4228-11eb-14fb-bd4ac7b0ce95
frequencies_plot_with_mean(do_experiment(p, 10_000))
...
# ╔═╡ 2d5ff412-4286-11eb-091f-3fae784ad5c6
function countingISR(agents, count_status)
	res = 0
	for i ∈ agents
		if i.status == count_status
			res += 1
		end
	end
	return res
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	...
	plot!(p, 1:T, sum_S[:, 1]./L, label="Susceptible")
	plot!(p, 1:T, sum_I[:, 1]./L, label="Infectious")
	plot!(p, 1:T, sum_R[:, 1]./L, label="Recovered")
	return p
end
...
@bind p_infection Slider(0.01:0.01:1, default=0.02, show_value=true)

# ╔═╡ 56dea1a6-428f-11eb-31ed-e75481d80b7a
@bind p_recovery Slider(0.001:0.001:1, default=0.002, show_value=true)
...
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
	...
	mean_R = sum_R ./ L
	for i in 1:L
		append!(var_S, [((simulations[i].S .- mean_S) .^2) ./L])
		append!(var_I, [((simulations[i].I .- mean_I) .^2) ./L])
		append!(var_R, [((simulations[i].R .- mean_R) .^2) ./L])
	end
	std_S = [sqrt.(arr) for arr ∈ var_S]
	std_I = [sqrt.(arr) for arr ∈ var_I]
	std_R = [sqrt.(arr) for arr ∈ var_R]
	plot!(p, 1:T, mean_S, label="Susceptible", ribbons=std_S, fillalpha=0.2)
	plot!(p, 1:T, mean_I, label="Infectious", ribbons=std_I, fillalpha=0.2)
	plot!(p, 1:T, mean_R, label="Recovered", ribbons=std_R, fillalpha=0.2)
	return p
end
...
# ╔═╡ c3359392-4296-11eb-338e-d1ae291dcef4
md"""
Yes
"""
...
begin
	...
	function interact!(agent::Agent, source::Agent, reinfection::Reinfection)
		if agent.status == S && source.status == I && bernoulli(reinfection.p_infection)
			agent.status = I
			source.num_infected += 1
		elseif agent.status == I && bernoulli(reinfection.p_recovery)
			agent.status = S
		end
	end
end
...
function step!(agents::Vector{Agent}, infection::AbstractInfection)
	# your code here
	randAgent, randSource = rand(1:length(agents), 2)
	interact!(agents[randAgent], agents[randSource], infection)
	return agents
end
...
function simulation(N::Integer, T::Integer, infection::AbstractInfection)
	...
	for i ∈ 1:T
		...
		append!(S_counts, countingISR(agents, S))
		append!(I_counts, countingISR(agents, I))
		append!(R_counts, countingISR(agents, R))
	end
...
let
	p = plot()
	sum = zeros(Int, 1, 1000)
	for sim in simulations
		plot!(p, 1:1000, sim.I, alpha=.5, label=nothing)
		sum = sum .+ sim.I
	end
	plot!(p, 1:1000, sum[:, 1] ./ length(simulations), lw=3, label="mean")
	p
end
...
# ╔═╡ 655a56ec-4291-11eb-046c-d340d5f4b5a5
sir_mean_error_plot(simulations)

# ╔═╡ 842ce98a-428f-11eb-26e0-c31eb738151b
let
...
function simulation_reinfection(N::Integer, T::Integer, reinfection::AbstractInfection)
	# your code here
	agents = generate_agents(N)
	S_counts = []
	I_counts = []
	R_counts = []
	for i ∈ 1:T
		agents = sweep!(agents, reinfection)
		append!(S_counts, countingISR(agents, S))
		append!(I_counts, countingISR(agents, I))
		append!(R_counts, countingISR(agents, R))
	end
...
# ╔═╡ ac0a4fd0-429a-11eb-0176-19677ac9b8ef
rein_simulations = repeat_simulations(100, 1000, Reinfection(0.02, 0.002), 20)

# ╔═╡ a8c9211a-429b-11eb-1729-2917094ef167
let
	p = plot()
	sum = zeros(Int, 1, 1000)
	for sim in rein_simulations
		plot!(p, 1:1000, sim.I, alpha=.5, label=nothing)
		sum = sum .+ sim.I
	end
	plot!(p, 1:1000, sum[:, 1] ./ length(rein_simulations), lw=3, label="mean")
	p
end
...
sir_mean_error_plot(rein_simulations)
