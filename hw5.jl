...
# ╔═╡ 6134e2be-42e4-11eb-1c50-1ba7d37beb72
origin == Coordinate(0, 0)
...
function make_tuple(pos::Coordinate)
	return (pos.x, pos.y)
end
...
function trajectory(c::Coordinate, n::Int, L::Number)
	moves = rand(possible_moves, n)
	res = accumulate(+, moves; init=c)
	return collide_boundary.(res, L)
end
...
# ╔═╡ e78c845e-4350-11eb-09ce-d9f7d3a2caae
...
# ╔═╡ c1b3f49c-42ab-11eb-2ac6-f7b8885cfcca
...
# ╔═╡ f12a00fa-42e6-11eb-2bb7-bbc7b7d2fde5
...
causes_outbreak = CollisionInfectionRecovery(0.7, 0.000001)

# ╔═╡ 269955e4-0a46-11eb-02cc-1946dc918bfa
does_not_cause_outbreak = CollisionInfectionRecovery(0.2, 0.001)
...
function step!(agents::Vector, L::Number, infection::AbstractInfection)
	ranNum = rand(1:length(agents)) # Source agent
	agents[ranNum].position += rand(possible_moves)
	agents[ranNum].position = collide_boundary(agents[ranNum].position, L)
...
# ╔═╡ aee54f9e-434d-11eb-29bc-2bb24829bd41
...
let
	...
	res_R = []
	for i ∈ 1:k_sweep_max
		agents = sweep!(agents, L, causes_outbreak)
		resSIR = countingSIR(agents)
		push!(res_S, resSIR[1])
		push!(res_I, resSIR[2])
		push!(res_R, resSIR[3])
	end
	left = plot()
	plot!(left, res_S, label="S")
	plot!(left, res_I, label="I")
	plot!(left, res_R, label="R")
	...
	res_R = []
	for i ∈ 1:k_sweep_max
		agents = sweep!(agents, L, does_not_cause_outbreak)
		resSIR = countingSIR(agents)
		push!(res_S, resSIR[1])
		push!(res_I, resSIR[2])
		push!(res_R, resSIR[3])
	...
	right = plot()
	plot!(right, res_S, label="S")
	plot!(right, res_I, label="I")
	plot!(right, res_R, label="R")
...
scatter([a.social_score for a ∈ social_agents], [a.num_infected for a ∈ social_agents], xlabel="Social Score", ylabel="Number of Infected", label=nothing)
...
let
	...
	@gif for t in 1:Tmax
		...
		plot(left, right, size=(600, 300), title="Lockdown")
	end
end

# ╔═╡ 5ac3b942-4365-11eb-34be-c305e55f9ffa
let
	N = 50
	L = 40
	...
	@gif for t in 1:Tmax
		...
		plot(left, right, size=(600, 300), title="Without Lockdown")
	end
end
