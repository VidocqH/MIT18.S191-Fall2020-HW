...
function tangent_line(f, a, h)
	fds = finite_difference_slope(f, a, h)
	res_f = x -> fds * (x-a) + f(a)
	return res_f
end
...
function euler_integrate(fprime::Function, fa::Number, 
	...
	for i ∈ a0+h:h:last(T)
		push!(res, euler_integrate_step(fprime, last(res), i, h))
	end
	return res
end
...
@bind β_interactive Slider(0.1:0.1:1, default=0.3, show_value=true)

# ╔═╡ 34f8cf66-44d9-11eb-3cc6-6f4359331ea9
@bind γ_interactive Slider(0.01:0.01:1, default=0.15, show_value=true)

# ╔═╡ 5a74fd64-44d9-11eb-1bf2-d9fa7b3ca985
plot_sir!(plot(), sir_T, euler_SIR(β_interactive, γ_interactive, [0.99, 0.01, 0.00], sir_T))
...
function ∂x(f::Function, a, b)
	# return f
	g = a -> f(a, b)
	return finite_difference_slope(a -> f(a, b), a)
end
...
function ∂y(f::Function, a, b)
	...
	return finite_difference_slope(b -> f(a, b), b)
end
...
function gradient_descent_1d(f, x0; η=0.01, N_steps=1000)
	for i ∈ 1:N_steps
		x0 = gradient_descent_1d_step(f, x0; η)
	end
	return x0
end
...
function loss_dice(μ, σ)
	...
	for i ∈ 1:length(dice_x)
		sum += (gauss(dice_x[i], μ, σ) - dice_y[i]) ^ 2
	end
	return sum
end
...
found_μ, found_σ = let
	...
	# your code here
	gradient_descent_2d(loss_dice, 30, 1; η=0.05)
end
...
function loss_sir(β, γ)
	sir_0 = [0.99, 0.01, 0]
	sirs = euler_SIR(β, γ, sir_0, hw4_T)
	sum = [0, 0, 0]
	for i ∈ 1:length(hw4_T)
		sum += (sirs[i] .- hw4_results[i]) .^ 2
	end
...
found_β, found_γ = let
	...
	gradient_descent_2d(loss_sir, 0.02, 0.002;η=0.00000001)
end
