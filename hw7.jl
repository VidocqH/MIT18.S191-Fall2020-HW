...
# ╔═╡ e066902c-4611-11eb-0af5-0d096bcc38c4
reflect(philip.l, rand(ex_1_scene).normal)
...
function interact(photon::Photon, hit::Intersection{Wall})
	l = reflect(photon.l, hit.object.normal)
	p = hit.point
	res = Photon(p, l, photon.ior)
	return res
end
...
function intersection(photon::Photon, sphere::Sphere; ϵ=1e-3)
	...
	if d < 0
		return Miss()
	elseif d == 0
		root = (-b)/(2*a) # time needs to reach the intersection point
		if root ≤ ϵ
			return Miss()
		end
		return Intersection(sphere, root, photon.p + root * photon.l)
	else
		root_1 = (-b + sqrt(d))/(2*a)
		root_2 = (-b - sqrt(d))/(2*a)
		roots = []
		...
		intersections = [Intersection(sphere, root, photon.p + root * photon.l) for root ∈ roots]
		l = length(intersections)
		if l == 0
			return Miss()
		elseif l == 1
			return intersections[1]
		else
			if isless(intersections[1], intersections[2])
				return intersections[1]
			else
...
# ╔═╡ 33e1772e-4671-11eb-1bfc-e7735a09c932
miss_sphere = Sphere(
	[7, 6],
	2,
	1.5,
...
# ╔═╡ 55f21c74-4671-11eb-31f0-69867b9d1aeb
hit_sphere = Sphere(
	[3+5*philip.l[1], 0+5*philip.l[2]],
	2,
	1.5,
)
...
function interact(photon::Photon, hit::Intersection{Sphere})
	...
	if photon.ior == 1
		# Entering the sphere
		ior = hit.object.ior # ≠1.0
		l = refract(photon.l, normal, photon.ior, ior)
		p = hit.point
		res = Photon(p, l, ior)
		return res
	else
...
function trace(photon::Photon, scene::Vector{<:Object}, N)
	...
	interact = photon
	for i ∈ 1:N
		interact = step_ray(interact, scene)
		push!(res, interact)
	end
	return res
end
...
# ╔═╡ 1f9727f4-46d1-11eb-1f16-53f46f6081d6
path = trace(philip, ex_1_scene, mirror_test_ray_N)
...
# ╔═╡ a7c6530c-46d6-11eb-1af8-91f9e746df96
let
...
# ╔═╡ 8c3a90f4-46da-11eb-05d9-df2cb738e50c
first_lens_photon = Photon([0,0], [1,0], 1.0)

# ╔═╡ b82cbbcc-46da-11eb-3ede-4fbb1a43787d
second_lens_photon = Photon([0,-3], [1,0], 1.0)
...
let
	...
	path_first = accumulate(1:N; init=first_lens_photon) do old_photon, i
		step_ray(old_photon, scene)
	end
	path_second = accumulate(1:N; init=second_lens_photon) do old_photon, i
		step_ray(old_photon, scene)
	end
	
	line_first = [first_lens_photon.p, [r.p for r in path_first]...]
	plot!(p, first.(line_first), last.(line_first), lw=5, color=:red)
	line_second = [second_lens_photon.p, [r.p for r in path_second]...]
	plot!(p, first.(line_second), last.(line_second), lw=5, color=:green)
